...
template<>
struct std::hash<Brainlearn::Eval::NNUE::EvalFile> {
    std::size_t operator()(const Brainlearn::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        Brainlearn::hash_combine(h, evalFile.defaultName);
        Brainlearn::hash_combine(h, evalFile.current);
        Brainlearn::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
