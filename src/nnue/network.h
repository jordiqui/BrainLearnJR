...
struct std::hash<Brainlearn::Eval::NNUE::Network<ArchT, FeatureTransformerT>> {
    std::size_t operator()(
      const Brainlearn::Eval::NNUE::Network<ArchT, FeatureTransformerT>& network) const noexcept {
        return network.get_content_hash();
    }
};

template<>
struct std::hash<Brainlearn::Eval::NNUE::Networks> {
    std::size_t operator()(const Brainlearn::Eval::NNUE::Networks& networks) const noexcept {
        std::size_t h = 0;
        Brainlearn::hash_combine(h, networks.big);
        Brainlearn::hash_combine(h, networks.small);
        return h;
    }
};
